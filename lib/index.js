...
Builder.prototype.build = function(count) {
  if (count && 'number' === typeof count) {
    ...
    for (var i = 0; i < count; i++) {
      objects.push(apply(new this.factory.Constructor(), this.factory.template, this.traits, this.attrOverwrite));
    }
    return objects;
  } else {
    return apply(new this.factory.Constructor(), this.factory.template, this.traits, this.attrOverwrite);
  }
};
